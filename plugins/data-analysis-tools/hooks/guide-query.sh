...
case "$tool_name" in
  *ask_deep_analysis*)
    cat <<'EOF'
{"systemMessage": "You are about to run a deep analysis query. If you have not already loaded the 'data-analysis-tools:query' skill, invoke the Skill tool with skill 'data-analysis-tools:query' BEFORE proceeding. The skill explains the three query methods (structured, natural language, deep analysis) and their correct parameters. For filter expressions, also load 'data-analysis-tools:filtering'."}
EOF
    ;;
  *)
    cat <<'EOF'
