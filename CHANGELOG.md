...
- **Frame deep analysis as an analyst sub-agent you delegate to** — the `query` skill documented `initiate_analysis` as a tool to call, so its rules (redirect instead of aborting, fresh conversation on topic change, follow up with `conversation_id`) read as API conventions with nothing behind them. Method 2 now opens by describing deep analysis as a stateful, resumable analyst sub-agent that plans its own investigation and is reached through a polling loop rather than a single call, which is what those rules follow from — including why it is not the tool for fetching one number.
- **Ask for the goal, not the plan** — nothing told the caller how to phrase the question, and an over-constrained question suppresses the semantic-layer context that is the reason to use deep analysis at all: the prescribed steps may be wrong because the caller lacks that context, and a question containing a full plan gives the analyst little reason to load it. Adds an `Asking the Question` section, mirrors the rule in `model-exploration` at the handoff point, and rewrites the two worked examples that prescribed dimensions and correlations to the analyst.
- **Document what to do once inside an analysis** — the skill covered starting an analysis but not the choice each follow-up presents, so a conversation would be opened and then hand-driven with `get_data_from_fields`. Adds `Continuing Inside an Analysis`: follow up when the next step needs reasoning or reuses groups the analysis already computed (structured queries cannot express ranks, table calculations, or filtering on a rank), drop out for deterministic slices. Records the investigate → query direction in `Combining Methods` — a finished analysis is the best source of exact field names for a structured query.
- **Document abort as an interrupt, not a cancel** — the section covered aborting and resuming without naming `abort_analysis`, and told the reader to let a misdirected analysis finish and redirect afterwards. Since abort stops the run without losing conversation state and the question sent on resume is the correction, interrupt-and-resume is the way to steer an analysis mid-flight; letting a wrong direction run to completion is the more expensive path. Self-initiated aborts are now gated on evidence — a misread `interpretation`/`plan`, a `step_insight` working from a false premise, or new information from the user — with the explicit caveats that a step whose purpose is not yet obvious is not evidence, and that being slower than you would have been is not grounds to interrupt.
- **Stop pointing debugging at the generated SQL** — `get_sql_from_fields` was described as useful for "review, debugging", and a best practice said to inspect the generated query, but its output is long machine-generated SQL and the model it was compiled from is the better source for what a field computes. It is now scoped to the cases where the SQL itself is what's wanted — handing it to the warehouse, dbt, or another tool, or bisecting a failing query — and debugging points at `get_field` / `get_entity`.
- **Gate `provide_analysis_feedback` on evidence rather than satisfaction** — it sat inline in the analysis lifecycle as a closing step, which reads as something to complete, and an agent rating an analysis it asked for itself is self-assessment. It now has its own section stating that there is one entry per conversation and a second call overwrites, so the user's judgement always wins; that agent-initiated feedback is allowed only from a check it can name — disagreement with or confirmation by an independent source, a data defect visible in the result, a conclusion the numbers do not support, a misleading chart — tagged `[agent]` with the evidence stated and `<Reason>:` kept leading so negative feedback stays machine-readable; and that leaving no feedback is a correct outcome, not a skipped step.
