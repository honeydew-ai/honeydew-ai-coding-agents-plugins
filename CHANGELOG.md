...
- **The guide hooks now require their skill instead of suggesting it** — a `PreToolUse` hook cannot make the model read a skill before the tool runs by injecting context: `additionalContext` is delivered with the tool *result*, after the call it was attached to has already executed, so the old "load the skill BEFORE proceeding" text described ordering the mechanism could not deliver. The hooks now return `permissionDecision: "deny"` when a required skill is not loaded, which blocks the call and hands the reason back for the model to act on and retry. Query, exploration and typed create/update calls are gated this way.
- **A block happens at most once per session, per skill** — the deny claims the session marker, so a model that retries without loading the skill is let through and can never be locked out of Honeydew. The reason text says so explicitly, so a model that cannot load the skill does not conclude Honeydew is unavailable.
- **Anything uncertain allows the call** — a skill that is not installed alongside the hooks, state that cannot be recorded, an unwritable `TMPDIR`, a payload that will not parse, a missing `session_id`: each of these lets the tool through rather than blocking work on a hook's bad day. `create_object` calls whose YAML matches no known object type stay advisory, naming the six candidate skills once per session without blocking, since there is no specific skill to require.

### Fixed

- **Guide hooks no longer repeat themselves or print into the transcript** — the hooks emitted `systemMessage`, which renders in the user's transcript, and kept no state, so every matching Honeydew MCP tool call re-printed the same skill reminder for the rest of the session; four consecutive queries produced four identical notices. Guidance now fires once per session per skill, and the remaining advisory path uses `additionalContext` with `suppressOutput`, which reaches the model without rendering in the UI.
- **A skill already in context produces no interruption at all** — the hooks check `transcript_path` for the skill, matching either the `Skill` tool_use record or the `<command-name>` slash-command form. Both structured forms are matched deliberately rather than the bare skill name: the hook's own reason text contains the skill name, so a loose match would hit its earlier output and disable the check permanently without the skill ever loading.
- **Deep analysis was never covered** — the query matcher listed `ask_deep_analysis_question`, which this MCP server does not expose, so `initiate_analysis` matched no hook and that branch was unreachable. The matcher now names `initiate_analysis`.
- **A batch of parallel tool calls no longer defeats the once-per-session gate** — the marker was created with a check-then-write, so concurrent invocations from a single batched assistant message all saw "not yet claimed" and all fired; six parallel calls produced six notices. Claiming is now an atomic `mkdir`.
- **Stale-state cleanup no longer deletes the live session's markers** — the weekly sweep used `find -maxdepth 1 -type d`, which matches the start directory itself, so it could `rm -rf` the entire marker root. It now passes `-mindepth 1`.
- **Hooks no longer fail on unexpected stdin** — `guide-exploration.sh` was a bare heredoc that could not fail; routing it through shared parsing gave it a hard dependency on well-formed JSON, and empty or malformed input exited 1, surfacing a hook error on every exploration call. Parsing now degrades to empty strings.
- **Cheaper per call** — the marker is checked before the transcript scan, so a session scans the transcript at most once per skill, and the two transcript patterns are matched in a single `grep` pass rather than two. Payload parsing is down to one `jq` call per script. Shared logic moved to `hooks/lib.sh`; object-type detection and all five YAML→skill mappings are unchanged.
