...
- **Cheaper on the repeat path** — the marker is checked before the transcript scan, so calls after the first cost a `stat()` instead of a grep, and the two transcript patterns are matched in a single `grep` pass rather than two. Payload parsing is down to one `jq` call per script (`guide-creation.sh` makes two more: one for object-type detection, one to emit). A repeat call drops from roughly 344 ms to 204 ms. Shared logic moved to `hooks/lib.sh`; object-type detection and all five YAML→skill mappings are unchanged.
- **Deep analysis gets guidance at all** — the query matcher listed `ask_deep_analysis_question`, which this MCP server does not expose, so `initiate_analysis` matched no hook and the deep-analysis branch was unreachable. The matcher now names `initiate_analysis`.
- **A batch of parallel tool calls no longer defeats the once-per-session gate** — the marker was created with a check-then-write, so concurrent invocations from a single batched assistant message all saw "not yet claimed" and all emitted. Six parallel calls produced six reminders. Claiming is now an atomic `mkdir`.
- **Stale-state cleanup no longer deletes the live session's markers** — the weekly sweep used `find -maxdepth 1 -type d`, which matches the start directory itself, so it could `rm -rf` the entire marker root. It now passes `-mindepth 1`. Failing to record state keeps the hook silent rather than falling back to firing on every call.
- **Hooks no longer fail on unexpected stdin** — `guide-exploration.sh` was previously a bare heredoc that could not fail; routing it through shared parsing gave it a hard dependency on well-formed JSON, and empty or malformed input exited 1, surfacing a hook error on every exploration call. Parsing now degrades to empty strings.
- **Reminder wording matches the mechanism** — `additionalContext` is delivered with the tool result, after the call it was attached to has run, so a reminder cannot gate that call. The texts now say "before your next call" rather than "BEFORE proceeding", and `hd_emit` sets `suppressOutput` to state the no-UI-rendering guarantee explicitly. Each query-tool branch names the `filtering` skill, since only one reminder fires per session and whichever tool runs first has to carry the whole hint.
